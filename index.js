...
var x = artists[8].name;
console.log(x.replace("Vincent Van Gogh"));
...
function getArtistByIndex(receivedArray, receivedNumber) {
  return `the artist at index ${receivedNumber} is ${receivedArray[receivedNumber].name}`;
}
...
function get20s(receivedArray = artists) {
  let y = [];
  for (let i = 0; i < receivedArray.length; i++) {
    let x = receivedArray[i].years.split(" ");
    x[(0, 2)] = x[0].trim();
    x[(0, 2)] = parseInt(x[0]);
    if (x[(0, 2)] > 1900 && x[(0, 2)] < 2000) {
      y.push(receivedArray[i].name);
    }
  }
  return y;
}
...
function removeArtist(receivedArray, receivedNumber) {
  receivedArray.splice(receivedNumber, 1);
  return receivedArray.length;
}
...
function addArtist(receivedArray) {
  const x = {
    id: 20,
    name: "Jamaria Sims",
    years: "1993 - 2021",
    ...
    bio:
      "Im a coding magician with a mission to help the world. Coding has always been my passion and will always be.",
  };

  receivedArray.push(x);
...
function lotsOfArt(newReceivedArray) {
  let newList = [];
  for (let i = 0; i < newReceivedArray.length; i++) {
    if (newReceivedArray[i].paintings > 100) {
      newList.push(newReceivedArray[i].name);
    }
  }
  console.log(newList);
  return newList;
